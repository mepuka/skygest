...
describe("entity search projector", () => {
  it("projects one document per in-scope entity with denormalized lineage and URL fields", () => {
    ...
    const distributionDoc = byTypeAndLabel("Distribution", "EIA Grid Monitor API");
    const seriesDoc = byTypeAndLabel("Series", "ERCOT wind generation (hourly)");
    ...
    expect(datasetDoc?.canonicalUrls).toContain("eia.gov/electricity/gridmonitor");
    ...
    expect(variableDoc?.lineageText).toContain("ERCOT wind generation (hourly)");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(new Set(docs.map((doc) => doc.entityId)).size).toBe(expectedCount);
