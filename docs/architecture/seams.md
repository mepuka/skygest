...
**4. `@skygest/DataLayerRegistry` lookup contract.** Stage 1, the kernel, and future lookup tools all depend on this one prepared registry surface. The code path for agent narrowing now exists, but the backing shelves are still incomplete until `SKY-317`, so changes here need extra care.
...
**6. Checked-in registry plus sync pipeline plus generated energy profile.** This is the source material behind the runtime registry and the kernel's facet semantics. Drift here produces quiet resolver quality regressions that look like runtime bugs.
...
- Controls the admin API, cache sync, registry sync, energy-profile generation, kernel eval runs, and deploys.
...
3. The generated energy-profile seam is now called out explicitly because it became load-bearing for the kernel.
4. The kernel eval harness is treated as a first-class architecture seam because it is now the active quality loop.
